...
import { getSeasonLeaderboard, getRaceLeaderboard, getScoredRounds, getUpcomingRace, getPredictionComparison, triggerAutoScore } from "../api/pitwall";
import F1Loader from "../components/F1loader";
import type { RaceEntry, SeasonEntry } from "../types";
import { ProcessingBanner } from "./Home";

function Avatar({ url, username }: { url: string | null; username: string }) {
  if (url) {
    return <img src={url} alt={username} className="w-8 h-8 rounded-full object-cover border border-zinc-700" />;
  }
  return (
...
function PickCell({ pick, actual }: { pick: string | null; actual: string | null }) {
  ...
  return <span className={`text-xs font-bold px-2 py-0.5 rounded ${correct ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-zinc-800 text-zinc-400"}`}>{pick}</span>;
}
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center z-10">
      <div className="bg-zinc-950/90 border border-zinc-800 rounded-2xl p-8 text-center max-w-sm mx-4 backdrop-blur-sm">
        ...
        <p className="text-zinc-500 text-sm mb-6">Create an account to compete against other fans and track your rankings</p>
        <div className="flex flex-col gap-3">
          <button onClick={() => navigate("/login")} className="w-full bg-red-500 hover:bg-red-600 text-white font-black py-3 rounded-xl transition-colors cursor-pointer">
            Sign In
          </button>
          <button onClick={() => navigate("/login")} className="w-full border border-zinc-700 hover:border-zinc-500 text-zinc-400 hover:text-white font-bold py-3 rounded-xl transition-all cursor-pointer">
            Create Account
          </button>
        </div>
      </div>
      ...
      <div className="relative border border-zinc-800 rounded-2xl bg-zinc-950 overflow-hidden">
        ...
        <div className={`divide-y divide-zinc-900 ${!isAuthed ? "blur-sm pointer-events-none select-none" : ""}`}>
          {leaderboard.map((entry) => {
            ...
            return (
              <div key={entry.user_id} className={`grid grid-cols-12 gap-2 px-5 py-4 items-center transition-colors ${isCurrentUser ? "bg-red-500/5 border-l-2 border-red-500" : "hover:bg-zinc-900/50"}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 flex-wrap">
        {scoredRounds.map((r) => (
          <button key={r.round} onClick={() => setSelectedRound(r.round)} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all cursor-pointer ${activeRound === r.round ? "bg-red-500 text-white" : "border border-zinc-800 text-zinc-500 hover:border-zinc-600 hover:text-zinc-300"}`}>
            R{r.round} · {r.name}
          </button>
        ))}
        {scoredRounds.length === 0 && <p className="text-zinc-600 text-sm">No races scored yet</p>}
      </div>
    ...
    <div className="space-y-6">
      <div className="border-l-4 border-red-500 pl-5">
        <div className="flex items-center gap-2 mb-3">
          <span className="bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-full tracking-widest uppercase">2026 Season</span>
        </div>
      ...
      <div className="flex gap-2">
        {(["season", "race"] as const).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2 rounded-xl text-sm font-bold transition-all cursor-pointer ${activeTab === tab ? "bg-red-500 text-white" : "border border-zinc-800 text-zinc-500 hover:border-zinc-600 hover:text-zinc-300"}`}>
            {tab === "season" ? "Season Rankings" : "Race-by-Race"}
          </button>
        ))}
      </div>

      {activeTab === "season" ? <SeasonTab currentUserId={user?.id} isAuthed={isAuthed} /> : <RaceTab currentUserId={user?.id} isAuthed={isAuthed} />}
    </div>
  );
}
