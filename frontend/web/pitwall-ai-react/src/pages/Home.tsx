...
import { getUpcomingRace, getWeekendPredictions, getPredictionComparison } from '../api/pitwall'
  ...
  Promise.all([
    ...
    .then(([r, c]) => {
      ...
      getWeekendPredictions(r.circuit, r.location)
        .then(p => setSessionCount(p.session_count))
    })
    ...
    <div className="space-y-2">
      ...
      <div className="group relative overflow-hidden border border-zinc-800 rounded-2xl p-10 bg-zinc-950 hover:border-red-500 transition-all duration-300 cursor-pointer">
        ...
        <p className="text-zinc-400 text-lg max-w-xl">
          Real-time F1 predictions powered by live session data,
          driver skill ratings and 2026 car performance index.
          Updated after every FP, Sprint and Qualifying session.
        </p>
        <Link
          to="/race"
  ...
  <div className="group relative overflow-hidden border border-zinc-800 rounded-2xl p-8 bg-zinc-950 hover:border-red-500 transition-all duration-300">
    ...
    <div className="grid grid-cols-2 gap-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          {comparison.comparison?.map(c => (
            <div key={c.position} className={`flex items-center gap-3 rounded-xl px-4 py-2 border ${
              c.correct
              ...
              <span className={`text-xs font-bold ${c.correct ? 'text-green-400' : 'text-red-400'}`}>
    ...
    <p className="text-zinc-600 text-xs mt-6">
      PitWall AI got {comparison.correct_count}/{comparison.total} podium positions correct
    </p>
  </div>
)}
